...
PostHog analytics is integrated but remains dormant until a project token is
configured. See [ANALYTICS.md](./ANALYTICS.md) for activation, privacy controls,
the complete event catalog, and suggested funnels.
...
We use Swifty Beaver as Logging System for detailed and efficient logging. For implementation details, see `Log.swift` in the `Debugging` folder.
...
### SwiftUI Views
- **Policy:** All new UI components must be created using SwiftUI.
...
11. **Statically Typed Fonts and Colors**: Use the custom `AppFont` enum and `Color` extension for consistent typography and color schemes across the app.
    ...
    // In UIKit
    let label = UILabel()
    label.font = UIFont.avenir(size: 16, weight: .medium)
